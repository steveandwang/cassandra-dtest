...
class TestAuthUpgrade(Tester):
    ...
    def upgrade_with_internal_auth_test(self):
        ...
        cassandra.execute("CREATE USER klaus WITH PASSWORD '12345' SUPERUSER")
        cassandra.execute("CREATE USER michael WITH PASSWORD '54321' NOSUPERUSER")
        cassandra.execute("CREATE KEYSPACE ks WITH replication = {'class':'SimpleStrategy', 'replication_factor':1}")
        cassandra.execute("CREATE TABLE ks.cf1 (id int primary key, val int)")
        cassandra.execute("CREATE TABLE ks.cf2 (id int primary key, val int)")
        cassandra.execute("GRANT MODIFY ON ks.cf1 TO michael")
        cassandra.execute("GRANT SELECT ON ks.cf2 TO michael")
        ...
        cassandra = self.patient_cql_connection(node1, user='cassandra', password='cassandra')
        cassandra.execute('DROP TABLE system_auth.users')
        cassandra.execute('DROP TABLE system_auth.credentials')
        cassandra.execute('DROP TABLE system_auth.permissions')
    ...
    def upgrade_to_version(self, tag, nodes=None):
        ...
        for node in nodes:
            # node.set_source_repo('/home/sam/projects/cassandra')
